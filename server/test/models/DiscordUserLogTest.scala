...
class DiscordUserLogTest extends PlaySpec {
  ...
  "save DateTime in properJson" in {
    ...
    val userLogRecoverd: JsResult[DiscordUserLog] =
      Json.fromJson[DiscordUserLog](js)
    assertResult(JsSuccess(u).map(_.date.getMillis))(
      userLogRecoverd.map(_.date.getMillis)
    )
  ...
  "save and recovery DiscordUserHistory" in {
    val log =
      DiscordUserLog(Random.nextString(12), guildID, None, DateTime.now())
    val userHistory = DiscordUserHistory(
      DiscordID(Random.nextString(12)),
      "1343",
      Random.nextString(12),
      Seq(log)
    )
    val userHistoryRecovered =
      Json.fromJson[DiscordUserHistory](Json.toJson(userHistory))
    val mapper: DiscordUserHistory => String = _.discriminator
    assertResult(JsSuccess(userHistory).map(mapper))(
      userHistoryRecovered.map(mapper)
    )
  }
}
