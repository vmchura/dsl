...
class DiscordFileServiceImplTest extends PlaySpec with GuiceOneAppPerSuite {
  val service: DiscordFileService =
    app.injector.instanceOf(classOf[DiscordFileService])

  "A Discord File Service" should {
    "add file" in {
      val f = service.pushFileOnChannel(
        "728442814832312375",
        new File(
          "/home/vmchura/Games/starcraft-remastered/drive_c/users/vmchura/My Documents/StarCraft/Maps/Replays/BisuVsProts/Stork vs Bisu blue.rep"
        ),
        "comment",
        "fileName"
      )
