  ...
  "A TournamentBuilder job" should {
    "insertion and load of tournament" in {
      val queryExecution = for {
        creation          <- tournamentService.saveTournament(Tournament(1L,"1Str","DServer","TorunamentName",active = true))
        deletion        <- if(creation) tournamentService.dropTournament(1L) else Future.successful(false)
    ...
    "build correctly a tournament" in {
      val queryExecution = for {
        creation              <- job.buildTournament("736004357866389584","DeathfateStarLeague")
        tournament            <- creation.withFailure
        tournamentLoadedOpt   <- tournamentService.loadTournament(tournament.challongeID)
        tournamentLoaded      <- tournamentLoadedOpt.withFailure(TournamentNotBuild(tournament.challongeID))
        participantsJob       <- job.getParticipantsUsers(tournament.challongeID)
        (_, participants, _)  <- participantsJob.withFailure
        participantsRemove <- Future.sequence(participants.map(_.participantPK).map(participantsService.dropParticipant))
        tournamentRemove      <- tournamentService.dropTournament(tournament.challongeID)
    ...
    "throw tournament created" in {
      val queryExecution = for {
        ...
        creation2          <- job.buildTournament("736004357866389584","DeathfateStarLeague")
