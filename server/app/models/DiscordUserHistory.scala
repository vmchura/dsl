  ...
  implicit val dateFormat: OFormat[DateTime] = new OFormat[DateTime] {
    ...
    override def reads(json: JsValue): JsResult[DateTime] = {
      try {
        ...
        case e: Throwable =>
          val item: (JsPath, Seq[JsonValidationError]) = (
            JsPath(List(KeyPathNode("iso"))),
            Seq(JsonValidationError(e.toString))
          )
          JsError(Seq(item))
      }
    }
...
case class DiscordUserHistory(
    discordID: DiscordID,
    discriminator: DiscordDiscriminator,
    lastUserName: String,
    logs: Seq[DiscordUserLog]
)
object DiscordUserHistory {
  import ModelsJsonImplicits._
  implicit val jsonFormat: OFormat[DiscordUserHistory] =
    Json.format[DiscordUserHistory]
}
