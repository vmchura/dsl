...
class ReplayMatchDAOImpl @Inject() (val reactiveMongoApi: ReactiveMongoApi)
    extends ReplayMatchDAO {
  def collection: Future[JSONCollection] =
    reactiveMongoApi.database.map(_.collection("dsl.replays"))

  override def add(replayRecord: ReplayRecord): Future[Boolean] =
    collection
      .flatMap(
        _.update(ordered = true).one(
          Json.obj("replayID" -> replayRecord.replayID),
          replayRecord,
          upsert = true
        )
      )
  ...
  private def getMatchesByQuery(query: JsObject) = {
    collection.flatMap(
      _.find(query, Option.empty[ReplayRecord])
        .cursor[ReplayRecord]()
        .collect[List](-1, Cursor.FailOnError[List[ReplayRecord]]())
    )
  ...
  override def loadAllByMatch(
    ...
    val query = Json.obj("tournamentID" -> tournamentID, "matchID" -> matchID)
    getMatchesByQuery(query)
  ...
  override def find(replayID: UUID): Future[Option[ReplayRecord]] = {
    ...
    collection.flatMap(
      _.find(query, Option.empty[ReplayRecord]).one[ReplayRecord]
    )
  ...
  override def updateLocation(
    ...
    collection
      .flatMap(
        _.update(ordered = true).one(
          Json.obj("replayID" -> replayID),
          Json.obj("$set" -> Json.obj("matchName" -> cloudLocation)),
          upsert = true
        )
      )
      .map(_.ok)
  ...
  override def isRegistered(hash: String): Future[Boolean] = {
    ...
    collection
      .flatMap(_.find(query, Option.empty[ReplayRecord]).one[ReplayRecord])
      .map(_.nonEmpty)
  }
}
