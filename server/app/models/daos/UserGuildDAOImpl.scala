...
class UserGuildDAOImpl @Inject() (val reactiveMongoApi: ReactiveMongoApi)
  ...
  private def save(userGuild: UserGuild): Future[Boolean] =
    collection.flatMap(_.insert(ordered = true).one(userGuild)).map(_.ok)

  private def push(discordID: DiscordID, guildID: GuildID): Future[Boolean] =
    collection
      .flatMap(
        _.update(ordered = true).one(
          Json.obj("discordID" -> discordID),
          Json.obj("$push" -> Json.obj("guilds" -> guildID)),
          upsert = true
        )
      )
      .map(_.ok)
  ...
  override def addGuildToUser(
    ...
    find(discordID).flatMap {
      case Some(result) =>
        if (result.guilds.contains(guildID)) Future.successful(true)
        else push(discordID, guildID)
  ...
  override def all(): Future[Seq[UserGuild]] = {
    collection.flatMap(
      _.find(Json.obj(), Option.empty[UserGuild])
        .cursor[UserGuild]()
        .collect[List](-1, Cursor.FailOnError[List[UserGuild]]())
    )
  }

  override def guilds(): Future[Seq[GuildID]] =
    all().map(_.flatMap(_.guilds).distinct)
}
