...
class ReplayMatchController @Inject()(scc: SilhouetteControllerComponents,
  ...
  def parseReplay(): Action[MultipartFormData[Files.TemporaryFile]] = silhouette.SecuredAction.async(parse.multipartFormData){ implicit request =>
    def buildResult(message: Either[String,ReplayDescriptionShared]) = Future.successful{
      Ok(Json.obj("response" -> write(message)))
    }
    val result = request.body.file("replay_file").fold(buildResult(Left("Sin archivo enviado"))){ replay_file =>
      val message = for{
        jsonString <- fileParser.parseFile(replay_file.ref.toFile)
        replayParsed <- fileParser.parseJsonResponse(jsonString)
      }yield{
        replayParsed
      }
      buildResult(message)
