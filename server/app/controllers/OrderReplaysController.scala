...
@Singleton
class OrderReplaysController @Inject() (
  ...
  def view(tournamentID: Long, matchID: Long): Action[AnyContent] =
        ...
        case (menues, discriminator) =>
          ...
          for {
            ...
            tournament <- tournamentOpt match {
              ...
              case _ =>
                Future
                  .failed(new IllegalArgumentException("No tournament found"))
            }
            ...
            Ok(
              orderreplays(
                ...
                tournamentID,
                matchID,
                replays.filter(_.enabled),
                OrderGamesForm.form.fill(
                  Data(3, replays.filter(_.enabled).map(_.replayID).toList)
                )
              )
            )
          }
  ...
  def submit(tournamentID: Long, matchID: Long): Action[AnyContent] =
    silhouette.SecuredAction(WithAdmin()).async { implicit request =>
      OrderGamesForm.form.bindFromRequest.fold(
        _ =>
          Future.successful(
            Redirect(routes.OrderReplaysController.view(tournamentID, matchID))
          ),
        data => {
          replayService.createFoldersAntiSpoilers(
            tournamentID,
            matchID,
            data.bof,
            data.replayID
