...
class RelateChaDisUsers(
    firstParam: String,
    secondParam: String,
    container: String
) {
  private val first =
    read[Seq[BasicComparableByLabel]](UtilParser.safeString2Json(firstParam))
  private val second =
    read[Seq[BasicComparableByLabel]](UtilParser.safeString2Json(secondParam))
  case class PairMatch(
      chauser: BasicComparableByLabel,
      disuser: Option[BasicComparableByLabel],
      bindedCorrectly: FutureBinding[Either[String, Boolean]]
  )
  private val pairsEnabled = Vars(
    first.map(f =>
      PairMatch(f, None, FutureBinding(Future.successful(Right(false))))
    ): _*
  )

  @html
  private def getMessageLinked(fut: FutureBinding[Either[String, Boolean]]) =
    Binding {
      (fut.bind match {
        case None =>
          <p>Wait</p>
        case Some(Success(Left(error))) =>
          <p>{error}</p>
        case Some(Success(Right(true))) =>
          <p>Relacionado!</p>
        case Some(Success(Right(false))) =>
          <p>Error!!</p>
  ...
  private def pairComponent(pm: PairMatch) = {
    val listThisMatch = second
      .sortBy(s =>
        Util
          .LCSLength(pm.chauser.stringLabelNormalized, s.stringLabelNormalized)
      )
      .reverse
  ...
  def postRelation(
      f: BasicComparableByLabel,
      s: BasicComparableByLabel
  ...
  def buildRelation(pm: PairMatch, selected: BasicComparableByLabel): Unit = {
    ...
    if (indx >= 0) {
      ...
      pairsEnabled.value
        .update(indx, pm.copy(disuser = Some(selected), bindedCorrectly = fut))
    }
  }
}
